...
<head>
    <title>KidsCare - ผู้ปกครอง</title>
    <style>
        .center {
            height: 100%;
            display: flex;
...
<body id="page-top">
                    ...
                    <form id="parentForm" enctype="multipart/form-data" autocomplete="off">
                        <div class="card shadow mb-4">
                            <div class="card-header d-flex justify-content-center align-items-center py-3">
                                <h6 class="m-0 font-weight-bold text-info" id="parent_topic">เพิ่มข้อมูลผู้ปกครอง</h6>
                            </div>
                                ...
                                <div class="row">
                                    <div class="col-lg-12">
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="form-group col-sm-4 ">
                                                <label for="validationServer01" class="form-label">อีเมลล์</label>
                                                <div class="input-group ">
                                                    <div class="input-group-prepend">
                                                        <span class="input-group-text"><i class="bi bi-envelope-at-fill"></i></span>
                                                    </div>
                                                    <input type="email" class="form-control " name="email" id="email" placeholder="โปรดระบุอีเมลล์">
                                                </div>
                                            </div>
    ...
    <script>
        ...
        function showParentInfo() {
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (data) {
                        $('#parent_topic').html('แก้ไขข้อมูลผู้ปกครอง #' + obj.id + " " + obj.pname + obj.fname + " " + obj.lname);
                        if (obj.pic) {
        ...
        function getZipcode() {
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $('#zipcode').val(data);
                    $("#parentForm").bootstrapValidator('revalidateField','zipcode');
                }
            });
        ...
        $('#parentForm').bootstrapValidator({
            ...
            // Prevent submit form
            $('#parentForm').data('bootstrapValidator').resetForm();
            e.preventDefault();
            if (id) {
                updateParent();
            } else {
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (obj.status == 200) {
                                Swal.fire({
                                    icon: 'success',
                                    title: 'เพิ่มข้อมูลผู้ปกครองสำเร็จ!',
                                    showConfirmButton: false,
                                    timer: 1500
                                }).then((result) => {
                                    location.reload();
                                    //location.href = 'parent_info.php?parent_id=' + obj.lastid;
                                })
                            } else {
                                Swal.fire({
                                    icon: 'error',
